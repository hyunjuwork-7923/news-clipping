...
description: Produces the team's actual daily media-clipping format — two sections covering (1) 언론보도 (시·SDF 관련, general Seoul City/Seoul Design Foundation press, not just goods) and (2) 굿즈관련 (general Korean goods/merchandise trend news, not restricted to Seoul's own brand) — for yesterday's date only, all domestic outlets, rendered as a refined text-centric HTML page. Use this whenever the user asks for a "서울 클리핑", "시정 클리핑", "SDF 클리핑", "굿즈 뉴스클리핑", "오늘 클리핑 만들어줘" in a Seoul city-hall/SDF context, or wants the daily press/goods roundup the team normally publishes — even if they only mention one of the two sections. Do NOT trigger for one-off single-article requests, for general national news unrelated to Seoul city administration or the goods/merch beat, or for SNS/social-media monitoring (that section was dropped — public search can't reliably verify social post content, so this skill doesn't attempt it).
---

# Seoul Daily Clipping (Press · Goods, 2 Sections)

This mirrors a real internal product: a daily clipping with two distinct sections, not one
undifferentiated list. Section scope is not uniform — Section 1 is genuinely broad (any Seoul
City/SDF-relevant press, not just goods), and Section 2 is genuinely broad in a different way
(any notable Korean goods/merch story, not just Seoul's own brand). Don't try to force one
filtering rule across both — each section has its own bar, described below.

(An earlier version of this skill had a third section, SNS채널, for official social-media
posts. It was dropped: this environment can't reliably verify what a social account actually
posted on a given day — logged-out platform pages don't expose the feed, and public web
search only catches a post if some other article happens to describe it. Rather than keep
publishing a section that was mostly "확인 불가," it was cut. If a future environment has
real access to the relevant social accounts, it'd be worth re-adding — but don't reintroduce
it without that.)

## Common setup

Take today's date from the environment/system context in KST and subtract one day — that's
the target calendar date for both sections. State it once at the top of the output.
...
Not a card grid — a numbered, editorial text list matching the real format: two headed
sections ("01 언론보도 (市·SDF 관련)", "02 굿즈관련"), sub-numbered items within Section 1
(1.1, 1.2...), and a numbered list for Section 2. Use `assets/template.html` as the starting
structure — it's styled after a "Bain Red" consulting-deck look the user referenced (red
#CC0000 / navy #00253E, bold sans headlines, thin rule dividers, uppercase tracked eyebrows,
a big red stat number per section showing its item count). This replaced an earlier serif
editorial look — don't revert to serif headlines or a softer palette without the user asking.
...
- Both section headers are present, even on a section with zero items
...
- Section 2 items are genuine dated goods/merch news, unrestricted by brand, but still real
...
- Layout is a numbered text list, not a card grid, and stays legible even when a section is
  short
- No SNS/social-media section — that was intentionally removed, don't add it back
